...
public class DeleteAccountServlet extends HttpServlet {
    ...
    try {
        // Chuyển đổi accID từ String sang int
        int accID = Integer.parseInt(request.getParameter("accID"));
        AccountRep db = new AccountRep();

        boolean deleted = db.deleteAccount(accID);
        if (deleted) {
            response.sendRedirect("accountTable.jsp");  // Quay lại danh sách tài khoản sau khi xóa
        } else {
            response.getWriter().println("❌ Failed to delete account.");
        }
    } catch (NumberFormatException e) {
        response.getWriter().println("❌ Invalid accID.");
    } catch (Exception e) {
