...
public class DeleteBookingServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            // Chuyển đổi bookingDetailID từ String sang int
            int bookingDetailID = Integer.parseInt(request.getParameter("bookingDetailID"));
            BookingDetailDB db = new BookingDetailDB();

            boolean deleted = db.deleteBooking(bookingDetailID);
            if (deleted) {
                response.sendRedirect("bookingTable.jsp");  // Quay lại danh sách đặt chỗ sau khi xóa
            } else {
                response.getWriter().println("❌ Failed to delete booking.");
            }
        } catch (NumberFormatException e) {
            response.getWriter().println("❌ Invalid bookingDetailID.");
        } catch (Exception e) {
